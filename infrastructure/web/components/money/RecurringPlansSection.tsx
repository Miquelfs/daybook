...
export function RecurringPlansSection({ plans, holdings, liquidAccounts }: Props) {
  ...
  const { open, toggle } = useCollapsible("portfolio-plans");
  ...
  return (
    <section className="mb-6">
      <div className="flex items-baseline justify-between mb-3 gap-3">
        <button type="button" onClick={toggle} className="flex items-baseline gap-3 min-w-0 group">
          <span className="text-xs text-[#52525B] uppercase tracking-widest group-hover:text-[#A1A1AA] transition-colors">
            Recurring plans {plans.length > 0 && <span className="normal-case tracking-normal">({plans.length})</span>}
          </span>
          {monthlyTotal > 0 && (
