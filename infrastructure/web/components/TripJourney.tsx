...
export function TripJourney({ dates, geojson }: { dates: string[]; geojson: TracksGeoJSON }) {
  ...
  const stops = useMemo(() => {
    const out: { date?: string; name: string; city: string | null; start: string; end: string; semantic: string | null }[] = [];
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <LocationMap key={selected} geojson={filtered} showLabels={selected !== "all"} />
      ...
      {stops.length > 0 ? (
        <div className="flex flex-col gap-1">
          ...
          {stops.map((s, i) => (
            <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-[#18181B] transition-colors">
