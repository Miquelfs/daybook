...
import { Plane, ArrowLeft, Clock, Calendar, MapPin, Moon } from "lucide-react";
...
export default function AircraftPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["aircraftHistory", reg],
    queryFn: () => api.aircraftHistory(reg),
    enabled: !!reg,
  });
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-[#09090B] p-6">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-[#71717A] hover:text-[#FAFAFA] mb-6">
          <ArrowLeft size={14} /> Back
        </button>
        <p className="text-[#71717A]">No flights found for <span className="font-mono text-[#FAFAFA]">{reg}</span>.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#09090B] text-[#FAFAFA]">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex items-start gap-4 mb-8">
          <div className="w-10 h-10 rounded-xl bg-violet-950/40 flex items-center justify-center flex-shrink-0">
            <Plane size={18} className="text-violet-400" />
          </div>
          <div>
            <h1 className="text-2xl font-bold tracking-tight font-mono">{data.registration}</h1>
            <p className="text-[#71717A] text-sm mt-0.5">{shortType(data.aircraft_type)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            { label: "Flights", value: String(data.total_flights), icon: <Plane size={12} /> },
            ...
            { label: "First flown", value: data.first_flight, icon: <Calendar size={12} /> },
        ...
        <div className="bg-[#18181B] border border-[#27272A] rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[#27272A]">
            <h2 className="text-sm font-medium text-[#A1A1AA]">All flights on this aircraft</h2>
          </div>
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      {(depCity || arrCity) && (
                        <div className="hidden sm:flex items-center gap-1 text-xs text-[#52525B] truncate">
                          <MapPin size={10} />
